...
public class ITestDataTable implements IDataTable<Test>
{
	...
	{
		RegaDBMain.getApp().getTree().getTreeContent().testSelected.setSelectedTest(selectedItem);
	...
	public boolean stillExists(Test selectedItem) 
	{
		Transaction trans = RegaDBMain.getApp().createTransaction();
        boolean state = trans.testStillExists(selectedItem);
        trans.commit();
        return state;
	}
}
